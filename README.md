...
- **微信 ↔ 飞书双向桥接**：消息自动双向转发，支持文本、图片等多种类型，无需人工干预
- **自然语言操作飞书**：在微信中说"帮我查一下今天的日程"，Hub AI 自动调用飞书 API 完成操作
- **34 个 AI Tools，覆盖 11 大业务域**：即时通讯、日历、云文档、任务、通讯录、云空间、多维表格、电子表格、邮箱、知识库、视频会议
- **WebSocket 实时事件**：基于飞书官方 SDK 长连接，实时接收飞书消息，无需配置公网回调地址
...
- `/send_lark_message --chat_id oc_xxx --text 你好`
...
在 [OpeniLink Hub](https://github.com/openilink/openilink-hub) 的应用市场中搜索「飞书」，一键安装即可使用，无需自行部署。
...
```bash
git clone https://github.com/openilink/openilink-app-lark.git && cd openilink-app-lark
npm install
cp .env.example .env
# 编辑 .env 填入你的配置
...
<details>
<summary><strong>飞书应用配置指南</strong></summary>
